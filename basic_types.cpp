...
Hash::Hash(char data[],size_t size) {
  ...
  c.Final((CryptoPP::byte*)this->bytes);
}
...
bool Sig::valid(char *data,size_t len) {
  ...
  publicKey.Load(
    *new CryptoPP::ArraySource((CryptoPP::byte*)this->signer->bytes, sizeof(this->signer->bytes), true /*pumpAll*/)
  );
  ...
  bool result = verifier.VerifyMessage(
    ...
    (const CryptoPP::byte*)this->bytes, sizeof(this->bytes)
  );
  return result;
}
